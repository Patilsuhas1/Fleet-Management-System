...
const Booking = () => {
    ...
    const [cars, setCars] = useState([]); // eslint-disable-line no-unused-vars
                ...
                <div className="row g-4">
                    {cars.length > 0 ? cars.map(car => (
                        <div className="col-md-4" key={car.carId}>
                            <div className="premium-card h-100 p-0 shadow-lg border-0">
                                <img src={car.imagePath || 'https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=500&q=60'}
                                    className="card-img-top" alt={car.carModel} style={{ height: '220px', objectFit: 'cover' }} />
                                <div className="p-4">
                                    <h5 className="fw-bold mb-1">{car.carModel}</h5>
                                    <p className="text-muted small mb-4">{car.carType?.carTypeName || 'Sedan'}</p>
                                    <div className="d-flex justify-content-between align-items-center">
                                        <div className="d-flex flex-column">
                                            <span className="small text-muted mb-0">Daily Rate</span>
                                            <span className="h4 fw-bold text-primary mb-0">₹{car.carType?.dailyRate || 5000}</span>
                                        </div>
                                        <button className="btn btn-premium btn-sm rounded-pill px-4" onClick={() => { setSelectedCar(car); setStep(3); }}>
                                            Select
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div className="col-12 text-center py-5">
                            <p className="text-muted mb-4">No cars available for the selected dates. Please try another range.</p>
                            <button className="btn btn-outline-primary rounded-pill px-4" onClick={() => setStep(1)}>Go Back</button>
                        </div>
                    )}
                </div>
            )}
